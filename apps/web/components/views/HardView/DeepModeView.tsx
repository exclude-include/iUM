...
import { Plus, ExternalLink, Sparkles, GripVertical, Trash2 } from "lucide-react";
...
export function DeepModeView() {
    ...
    return (
        <div className="flex flex-col h-full min-h-0 bg-background deep-mode-view overflow-hidden">
            <div className="p-3 border-b flex items-center justify-between text-xs text-muted-foreground bg-muted/20 shrink-0">
                <p>Select text in the main tab and right-click to "Deep Dive".</p>
                {deepHistory.length > 0 && (
                    <Button
                        variant="ghost"
                        size="sm"
                        className="h-6 px-2 text-[10px] hover:text-destructive"
                        onClick={clearDeepHistory}
                    >
                        ...
                        {deepHistory.map((item) => (
                            ...
                            >
                                <Card className="overflow-hidden border-primary/20 shadow-sm">
                                    <div className="bg-primary/5 p-2 px-3 flex items-center justify-between border-b border-primary/10">
                                        ...
                                        <div className="flex gap-1">
                                            ...
                                            <Button
                                                ...
                                                onClick={() => deleteDeepCard(item.id)}
                                                title="Delete card"
                                            >
                                    ...
                                    <div className="p-3 text-sm prose prose-sm dark:prose-invert max-w-none break-words cell-content">
                                        <ReactMarkdown
